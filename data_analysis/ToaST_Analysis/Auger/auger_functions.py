...
def base_correct2(energy,readout,n=5):
    p = np.polyfit(energy[900:],readout[900:],n)
    myfit = [sum([p[i]*x**(n-i) for i in range(n+1)] ) for x in energy]
##    plt.plot(energy,readout)
##    plt.plot(energy,myfit)
##    plt.figure()
#    plt.show()
    return energy,readout-myfit

def base_correct(energy,readout,n=3):
    #fourier transform and filter out low frequency
    freadout = np.fft.rfft(readout)
    newreadout = np.fft.irfft(freadout[n:],n=len(energy))
    #plt.figure()
    #plt.plot(newreadout)
    #plt.show()
    newenergy = energy
    return newenergy,newreadout
def normalize(energy,readout,nstart,nend,peaks):
    mypeaks = []
    for peak in peaks:
        [tempen,tempheight] = peak
        myen = energy[tempen]
        mypeaks.append([myen,tempheight])
        if nstart < myen < nend:
            #this is the carbon peak
            N = tempheight
    readout = readout/abs(N)
    return energy,readout,mypeaks

def find_peaks(energy,readout):
    return peakdetect(readout)[1]
...
def import_data(filename,nstart=260,nend=280,basecorrect=1):
    ...
    readout = data[:,1]*1000.
    if basecorrect:
        (energy,readout) = base_correct2(energy,readout,n=2)
    peaks = find_peaks(energy,readout)
    ##uncomment next line to normalize to Carbon by default
 ##   (energy,readout,mypeaks) = normalize(energy,readout,nstart,nend,peaks)

    return energy,readout,peaks

def calc_ratios(peaks1,peaks2):
    ratios = []
    result = []
    for x in range(len(peaks1)):
        for y in range(len(peaks2)):
            if peaks2[y][0]-1 < peaks1[x][0] < peaks2[y][0]+1:
                result.append(peaks1[x][0])
                ratios.append(peaks1[x][1]/peaks2[y][1])
